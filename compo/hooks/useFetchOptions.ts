...
const useFetchOptions = ({ labels, collaborationId, pNumber, data, selectedCards, stateFilterValues, searchTerms }: FetchOptionsParams) => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      // Fetch platforms
      const platformResponse = await getPlatforms(collaborationId, pNumber);
      ...
      for (const label of labels) {
        ...
        if (searchTerm) {
          const response = await getSocMedFilter(collaborationId, pNumber, { ...data, searchTerm }, label.type);
